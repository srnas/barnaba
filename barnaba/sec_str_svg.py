...
def draw_structure(pos, pairs, ann_list, sequence, residue_numbers, dimensions, output_ids):
	binary = False
	n = len(pos)
	output_svg = ""
	...
	for i, p in enumerate(pos):
		if output_ids:	
			output_svg += draw_res(p, residue_numbers[i])
		else:	
			output_svg += draw_res(p, sequence[i])	
...
<line x1="%.4f" y1="%.4f" x2="%.4f" y2="%.4f" stroke="rgb(25%%, 25%%, 25%%)" stroke-width="1.0" />''' % (pos[0][0], pos[0][1]+12.5, pos[0][0], pos[0][1]+7.5)
	output_number += '''
<text x="%.4f" y="%.4f" text-anchor="middle" font-family="Liberation Sans" font-size="7.5" fill="rgb(25%%, 25%%, 25%%)" >%d</text>''' % (pos[0][0], pos[0][1]+23, res_shift)
	output_number += '''
<line x1="%.4f" y1="%.4f" x2="%.4f" y2="%.4f" stroke="rgb(25%%, 25%%, 25%%)" stroke-width="1.0" />''' % (pos[-1][0], pos[-1][1]+12.5, pos[-1][0], pos[-1][1]+7.5)
	output_number += '''
<text x="%.4f" y="%.4f" text-anchor="middle" font-family="Liberation Sans" font-size="7.5" fill="rgb(25%%, 25%%, 25%%)" >%d</text>''' % (pos[-1][0], pos[-1][1]+23, len(pos)-1+res_shift)
	output_number += '''
<line x1="%.4f" y1="%.4f" x2="%.4f" y2="%.4f" stroke="rgb(25%%, 25%%, 25%%)" stroke-width="1.0" />''' % (pos[9][0]-12.5, pos[9][1], pos[9][0]-7.5, pos[9][1])
	output_number += '''
<text x="%.4f" y="%.4f" text-anchor="middle" font-family="Liberation Sans" font-size="7.5" fill="rgb(25%%, 25%%, 25%%)" >%d</text>''' % (pos[9][0]-20, pos[9][1]+3, 9+res_shift)
	output_number += '''
<line x1="%.4f" y1="%.4f" x2="%.4f" y2="%.4f" stroke="rgb(25%%, 25%%, 25%%)" stroke-width="1.0" />''' % (pos[19][0]+12.5, pos[19][1], pos[19][0]+7.5, pos[19][1])
	output_number += '''
<text x="%.4f" y="%.4f" text-anchor="middle" font-family="Liberation Sans" font-size="7.5" fill="rgb(25%%, 25%%, 25%%)" >%d</text>''' % (pos[19][0]+20, pos[19][1]+3, 19+res_shift)
	return output_number

def draw_res(xy, letter):
	x = xy[0]
	y = xy[1]
	output_res = '''
<circle cx="%.4f" cy="%.4f" r="5.0" stroke="#595959" stroke-width="1.0" fill="#ffffff"/>
<text x="%.4f" y="%.4f" text-anchor="middle" font-family="Liberation Sans" font-size="7.5" fill="#000000" >%s</text>''' % (x, y, x, y+2.7, letter)
	return output_res
